...
#### CHI-SQUARE TEST - NO-SITES####

# Lithology
Lithology_nosites <- data.frame ('Lithology' = nositestab$Lithology) %>%
  mutate(Lithology = factor(Lithology, levels = c('1','2','3','4'))) %>%
  count (Lithology, .drop = FALSE) 

Lithology_area_NS <- data.frame (Carona$Lithology) %>%
  group_by (Lithology) %>%
  summarize (pixels = n()) %>%
  mutate (perc = pixels/sum(pixels))%>%
  mutate (nosites = Lithology_nosites$n)

chisq.test(x=Lithology_area_NS$nosites,p=Lithology_area_NS$perc,simulate.p.value =  TRUE)

# Permeability
Permeability_nosites <- data.frame ('Permeability' = nositestab$Permeability) %>%
  mutate(Permeability = factor(Permeability, levels = c('1','2','3'))) %>%
  count (Permeability, .drop = FALSE) 

Permeability_area_NS <- data.frame (Carona$Permeability) %>%
  group_by (Permeability) %>%
  summarize (pixels = n()) %>%
  mutate (perc = pixels/sum(pixels))%>%
  mutate (nosites = Permeability_nosites$n)

chisq.test(x=Permeability_area_NS$nosites,p=Permeability_area_NS$perc,simulate.p.value =  TRUE)

# Avalanche
Avalanche_nosites <- data.frame ('Avalanche' = nositestab$Avalanche) %>%
  mutate(Avalanche = factor(Avalanche, levels = c('1','2','3'))) %>%
  count (Avalanche, .drop = FALSE) 

Avalanche_area_NS <- data.frame (Carona$Avalanche) %>%
  group_by (Avalanche) %>%
  summarize (pixels = n()) %>%
  mutate (perc = pixels/sum(pixels))%>%
  mutate (nosites = Avalanche_nosites$n)

chisq.test(x=Avalanche_area_NS$nosites,p=Avalanche_area_NS$perc,simulate.p.value =  TRUE)

# MorFeat
MorFeat_nosites <- data.frame ('MorFeat' = nositestab$MorFeat) %>%
  mutate(MorFeat = factor(MorFeat, levels = c('1','2','3','4','5','6'))) %>%
  count (MorFeat, .drop = FALSE) 

MorFeat_area_NS <- data.frame (Carona$MorFeat) %>%
  group_by (MorFeat) %>%
  summarize (pixels = n()) %>%
  mutate (perc = pixels/sum(pixels))%>%
  mutate (nosites = MorFeat_nosites$n)

chisq.test(x=MorFeat_area_NS$nosites,p=MorFeat_area_NS$perc,simulate.p.value =  TRUE)
